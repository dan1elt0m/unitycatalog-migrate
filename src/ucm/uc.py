...
from unitycatalog.client import ApiClient, Configuration, CatalogsApi, SchemasApi, CreateSchema, CreateCatalog, \
    TablesApi, CreateTable, ColumnInfo
...
async def create_catalog(workspace_client:WorkspaceClient, catalogs_api: CatalogsApi, name:str):
...
async def create_schema(workspace_client:WorkspaceClient, schemas_api:SchemasApi, name:str):
    """Create a schema in Unity Catalog"""
    s = workspace_client.schemas.get(full_name=name)
    if s.catalog_name == "system":
        raise SystemCatalogError()
    await schemas_api.create_schema(CreateSchema(
        name=s.name,
...
async def create_table(workspace_client:WorkspaceClient, tables_api:TablesApi, name:str):
    """Create a table in Unity Catalog"""
    t = workspace_client.tables.get(full_name=name)
    if t.catalog_name == "system":
        raise SystemCatalogError
    await tables_api.create_table(CreateTable(
        name=t.name,
