...
async def migrate_catalog(
    ...
    async with get_uc_client(token) as uc_client:
        tasks = [(name, create_catalog(w, CatalogsApi(uc_client), name)) for name in names]
...
async def migrate_schema(
    names: List[str] = typer.Argument(..., help="Schema names to migrate. Format: catalog_name.schema_name"),
    profile: str = typer.Option("default", help="Databricks Config profile"),
    token: str = typer.Option(None, help="UC Access token"),
):
    ...
    w = WorkspaceClient(profile=profile)
    async with get_uc_client(token) as uc_client:
        tasks = [(name, create_schema(w, SchemasApi(uc_client), name)) for name in names]
        await run_migration_tasks(tasks)
...
async def migrate_table(
    names: List[str] = typer.Argument(..., help="Table names to migrate. Format: catalog_name.schema_name.table_name"),
    profile: str = typer.Option("default", help="Databricks Config profile"),
    token: str = typer.Option(None, help="UC Access token"),
):
